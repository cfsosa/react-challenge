...
class RegisterPage extends React.Component {
	...
	handleSubmit(event) {
		...
		if(user.username){
			...
			if (lastAtPos < lastDotPos && lastAtPos > 0 && user.username.indexOf('@@') == -1 && lastDotPos > 2 && (user.username.length - lastDotPos) > 2) {
		...
		if (user.firstName && user.lastName && user.username && user.password && goto) {
	...
	render() {
		...
		return (
			...
			{submitted && !isvalid &&
				<div className="help-block text-right text-danger">* invalid email address</div>
			}
			</div>
